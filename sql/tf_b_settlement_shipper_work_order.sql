-- =====================================================================
-- 智运宝 - 货主清算工单表（tf_b_settlement_shipper_work_order）
-- =====================================================================
-- 设计说明：
--   1. 业务触发点：货主发起「结算申请」、货主清算单（tf_b_settlement_shipper）落库后，
--      系统以「工单」形式记录本条清算消息（延迟 30S）投递 MQ 并驱动清算执行的全过程：
--      待投递 -> 已投递 -> 处理中 -> 处理成功 / 处理失败（可重试）/ 已作废；
--      与 SettlementServiceImpl#applySettlement 中「货主清算工单生成后，在这里做货主清算功能，
--      走延迟队列，延迟30S」的落地位置一一对应。
--   2. 与 tf_b_settlement_shipper 的关系：一张货主清算单对应一张清算工单（1:1）。
--      工单冗余清算金额与双方快照，消费端可免回查、独立执行清算；
--      唯一索引 uk_settlement_no 兜底，防止重复建单。
--   3. 幂等性设计（三层）：
--      (1) 建单幂等：uk_idempotent_key（幂等键 = SHIPPER_CLEARING:{orderId}）唯一约束，
...
--          受影响行数为 0 即判定重复消费，直接 ACK 跳过，保证清算只执行一次。
...
--      实际资金动作仍由清算单 + 账户域在同一事务内完成（见 tf_b_settlement_shipper 设计说明），
--      工单不参与金额计算，仅记录执行结果。
...
DROP TABLE IF EXISTS `tf_b_settlement_shipper_work_order`;

CREATE TABLE `tf_b_settlement_shipper_work_order` (
      `id`                BIGINT        NOT NULL AUTO_INCREMENT COMMENT '主键ID',
      `work_order_no`     VARCHAR(32)   DEFAULT NULL            COMMENT '清算工单号（HJSG+流水：yyyyMMddHHmmss+序号）',
      `settlement_no`     VARCHAR(32)   DEFAULT NULL            COMMENT '货主清算单号（tf_b_settlement_shipper.settlement_no）',
      `order_id`          VARCHAR(64)   DEFAULT NULL            COMMENT '运单号（tf_b_order.order_id）',
      `idempotent_key`    VARCHAR(128)  DEFAULT NULL            COMMENT '业务幂等键（SHIPPER_CLEARING:{orderId}），建单幂等唯一约束',
      `shipper_user_id`   VARCHAR(64)   DEFAULT NULL            COMMENT '货主用户ID（冗余，清算归属方）',
      ...
      `payable_amount`    DECIMAL(16,2) DEFAULT 0.00            COMMENT '应付总额=运费+服务费-优惠+其他（清算金额快照）',
      ...
      `routing_key`       VARCHAR(128)  DEFAULT NULL            COMMENT 'MQ 路由键（business.settlement.shipper.clearing）',
      `queue_name`        VARCHAR(128)  DEFAULT NULL            COMMENT 'MQ 队列名（settlement_shipper_clearing_queue）',
      `message_id`        VARCHAR(64)   DEFAULT NULL            COMMENT 'MQ 消息唯一ID（工单创建时预生成，投递/消费统一幂等标识）',
      `message_type`      VARCHAR(64)   DEFAULT NULL            COMMENT '消息类型（SETTLEMENT_SHIPPER_CLEARING）',
...
-- =====================================================================
-- 示例数据（可按需执行）
-- 与 tf_b_settlement_shipper 示例保持一致：
--   HJS202609120001（YD20260902001，已结算）-> 工单处理成功
--   HJS202609120002（YD20260902002，待结算）-> 工单已投递，等待消费
-- 注：uk_message_id 唯一索引不约束多行 NULL，message_id 允许在建单时先置空、投递时回填。
-- =====================================================================
INSERT INTO `tf_b_settlement_shipper_work_order`
(`work_order_no`, `settlement_no`, `order_id`, `idempotent_key`,
 `shipper_user_id`, `shipper_user_name`, `shipper_name`, `shipper_mobile`,
 `carrier_user_id`, `carrier_name`, `carrier_mobile`,
 `transport_money`, `payable_amount`, `frozen_no`, `frozen_amount`,
...
VALUES
    ('HJSG202609120001', 'HJS202609120001', 'YD20260902001', 'SHIPPER_CLEARING:YD20260902001',
     'U001', '张三', '张三物流部', '13800000001',
     'U002', '李四运输队', '13900000002',
     1800.00, 1800.00, 'FZ20260902001', 1800.00,
     'monkey.business.exchange', 'business.settlement.shipper.clearing', 'settlement_shipper_clearing_queue',
     '9f2c1e6a-4b7d-4f1e-8a33-2c5d7b91e001', 'SETTLEMENT_SHIPPER_CLEARING', 'YD20260902001',
     '{"orderId":"YD20260902001","settlementNo":"HJS202609120001","payableAmount":1800.00}', 30,
     4, '处理成功', 0, 5,
     '2026-09-12 10:00:00', '2026-09-12 10:00:30', '2026-09-12 10:05:00', NULL, NULL, '延迟30S投递，货主托管运费已扣划完成', 'gkk'),
    ('HJSG202609120002', 'HJS202609120002', 'YD20260902002', 'SHIPPER_CLEARING:YD20260902002',
     'U001', '张三', '张三物流部', '13800000001',
     'U002', '李四运输队', '13900000002',
     2000.00, 2000.00, 'FZ20260902002', 2000.00,
     'monkey.business.exchange', 'business.settlement.shipper.clearing', 'settlement_shipper_clearing_queue',
     '9f2c1e6a-4b7d-4f1e-8a33-2c5d7b91e002', 'SETTLEMENT_SHIPPER_CLEARING', 'YD20260902002',
     '{"orderId":"YD20260902002","settlementNo":"HJS202609120002","payableAmount":2000.00}', 30,
     2, '已投递', 0, 5,
     '2026-09-12 11:20:00', NULL, NULL, NULL, NULL, '延迟30S投递，等待消费端执行清算', 'gkk');
...
--    FROM tf_b_settlement_shipper_work_order
--    WHERE delete_flag = 0
...
--    FROM tf_b_settlement_shipper_work_order
--    WHERE idempotent_key = 'SHIPPER_CLEARING:YD20260902001' AND delete_flag = 0;
--
-- 3) 消费幂等 CAS：仅当工单处于「已投递(2)」时推进为「处理中(3)」，
--    受影响行数为 1 才继续执行清算，为 0 表示已被消费 / 状态不符，直接 ACK 跳过。
--    UPDATE tf_b_settlement_shipper_work_order
--    SET status = 3, status_desc = '处理中', consume_time = NOW(), update_time = NOW()
--    WHERE work_order_no = 'HJSG202609120002' AND status = 2 AND delete_flag = 0;
--
-- 4) 处理成功回写
--    UPDATE tf_b_settlement_shipper_work_order
--    SET status = 4, status_desc = '处理成功', finish_time = NOW(), error_msg = NULL, update_time = NOW()
--    WHERE work_order_no = 'HJSG202609120002' AND status = 3 AND delete_flag = 0;
--
-- 5) 处理失败回写并安排重试（retry_count 达上限时由告警转入人工处理）
--    UPDATE tf_b_settlement_shipper_work_order
--    SET status = 5, status_desc = '处理失败', retry_count = retry_count + 1,
--        error_msg = '账户扣划超时', next_retry_time = DATE_ADD(NOW(), INTERVAL 30 SECOND), update_time = NOW()
--    WHERE work_order_no = 'HJSG202609120002' AND status = 3 AND delete_flag = 0;
-- =====================================================================
