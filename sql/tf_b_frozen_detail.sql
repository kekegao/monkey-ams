...
CREATE TABLE `tf_b_frozen_detail` (
  ...
  `biz_type`     TINYINT      NOT NULL DEFAULT 1      COMMENT '冻结业务类型：1运费托管 2提现冻结',
  `biz_type_name` VARCHAR(32) DEFAULT NULL            COMMENT '业务类型名称（冗余）：运费托管/提现冻结',
  `ref_id`       VARCHAR(64)  NOT NULL                COMMENT '关联业务ID（运单ID/提现申请ID）',
  `order_no`     VARCHAR(64)  NOT NULL                COMMENT '关联单号（YD运单号/TX提现单号）',
  `amount`       DECIMAL(16,2) NOT NULL               COMMENT '冻结金额（元），须大于0',
  `frozen_time`  DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '冻结时间',
  `status`       TINYINT      NOT NULL DEFAULT 1      COMMENT '状态：1冻结中/处理中 2已解冻(退回可用余额) 3已打款(提现完成)',
  ...
  `delete_flag`  TINYINT      NOT NULL DEFAULT 0      COMMENT '删除标记：0正常 1已删除',
